...
#m1.0 <- glmer(entrained~interlocutor.SES+(1 + interlocutor.SES|participantID)+(1 + interlocutor.SES|disfavoured.word), data =d.exp1, family ="binomial")  boundary (singular) fit: see ?isSingular

#m1.1 <- glmer(entrained~interlocutor.SES+(1 + interlocutor.SES|participantID)+(1 + interlocutor.SES||disfavoured.word), data =d.exp1, family ="binomial") boundary (singular) fit: see ?isSingular 

m1.2 <- glmer(entrained~interlocutor.SES+(1 + interlocutor.SES|participantID)+(1|disfavoured.word), data =d.exp1, family ="binomial")
summary(m1.2)
...
m3 <- glmer(entrained ~ interlocutor.SES*group+session.order+(1+interlocutor.SES|participantID)+(1|disfavoured.word), data = d.exp2, family ="binomial")
summary(m3)
```
...
main.plot <- ggplot(vis0, aes(x=group, y=entrainment.m, fill=interlocutor.SES)) +
...
**Additional findings: Attention (Session Order)**
```{r echo=TRUE, message=FALSE, warning=FALSE}
vis1 <- d0 %>%
...
plot.order <- ggplot(vis1, aes(x=experiment, y=entrainment.m, fill=session.order)) +
  labs(title="Lexical entrainment decreases over time", y="proportion of lexical entrainment (%)", x="experiment") +
  ylim(0, 50) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
...
Images can be found in the current folder.