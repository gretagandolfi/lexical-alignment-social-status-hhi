...
Visualisation
```{r}
d1 <- d_exp1 <- read.csv("/Users/gretagandolfi/Desktop/OneDrive - University of Edinburgh/ESR6/0DONE/_STUDY1/xprag-abstract/social-status-experiments-gender-info.csv")
d1$participantID <- as.factor(as.character(d1$prolific_id))
d1$interlocutor.SES <- as.factor(as.character(d1$condition))
d1$stimuli.order <- as.factor(as.character(d1$randomBlock))
d1$gender.participant <- as.factor(d1$gender_participant)
d1$rating.position <- as.factor(as.character(d1$group))
d1 <- d1 %>% 
