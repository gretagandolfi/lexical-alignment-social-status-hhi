...
ggplot(v3, aes(x=interlocutor.SES, y=entrainment.m, group=participantID)) + geom_point() + geom_line()  + facet_wrap(vars(participantID))
...
v2 <- d0 %>%
  ...
  dplyr::summarise(entrainment.m = mean(entrained)*100,
                   ...
                   N = n(),
                   entrainment.se = entrainment.sd/sqrt(n()))
ggplot(v2, aes(fill = interlocutor.SES, x = entrainment.m)) +
